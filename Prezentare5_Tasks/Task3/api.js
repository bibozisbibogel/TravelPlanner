...
const UsersAPI = {
  ...
  login: (email, password, rememberMe = false) => apiFetch('/users/login', { method: 'POST', body: JSON.stringify({ email, password, rememberMe }) }),
  ...
  uploadAvatar: (id, formData) => fetch(`${API_BASE}/users/${id}/avatar`, { method: 'POST', body: formData, credentials: 'include' })
    .then(async r => { if (!r.ok) throw await r.json().catch(() => ({ message: r.statusText })); return r.json(); }),
};
