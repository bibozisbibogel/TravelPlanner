...
{
        // Fixed GUIDs so the frontend DEFAULT_USER always matches the DB record
    ...
    public static async Task SeedAsync(ApplicationDbContext context)
    {
        ...
        var demoUser = await context.Users.FindAsync(DemoUserId);
        if (demoUser == null)
        {
        ...
        else
        {
            ...
            if (!demoUser.PasswordHash.StartsWith("$2"))
            {
                demoUser.PasswordHash = BCrypt.Net.BCrypt.HashPassword("demo");
