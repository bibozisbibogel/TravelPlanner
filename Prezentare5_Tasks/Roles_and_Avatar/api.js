...
const UsersAPI = {
  ...
  getAll: (requesterId) => apiFetch('/users', { headers: { 'Content-Type': 'application/json', 'X-User-Id': requesterId } }),
  getById: (id) => apiFetch(`/users/${id}`),
  update: (id, data) => apiFetch(`/users/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  uploadAvatar: (id, formData) => fetch(`${API_BASE}/users/${id}/avatar`, { method: 'POST', body: formData })
